	...
	<head>
		...
		<script >
			so.init(function(){
				...
				<@shiro.hasPermission name="/api/role/deleteRoleById">
				//全选
			...
			function deleteById(ids){
				var index = layer.confirm("确定这"+ ids.length +"个角色？",function(){
					var load = layer.load();
					$.post('${basePath}/api/role/deleteRoleById',{ids:ids.join(',')},function(result){
						layer.close(load);
			...
			<@shiro.hasPermission name="/api/role/addRole">
			<#--添加角色-->
			function addRole(){
				var name = $('#name').val(),
					type = $('#type').val();
				...
				<#--loding-->
				var load = layer.load();
				$.post('${basePath}/api/role/addRole',{name:name,type:type},function(result){
					layer.close(load);
					if(result && result.status != 200){
