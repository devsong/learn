...
<head>
	...
	<script>
		so.init(function(){
				...
				<@shiro.hasPermission name="/api/member/deleteUserById">
				//全选
			...
			function _delete(ids){
				var index = layer.confirm("确定这"+ ids.length +"个用户？",function(){
					var load = layer.load();
					$.post('/api/member/deleteUserById',{ids:ids.join(',')},function(result){
						layer.close(load);
			...
			<@shiro.hasPermission name="/api/member/forbidUserById">
			/*
			*激活 | 禁止用户登录
			*/
			function forbidUserById(status,id){
				...
				var index = layer.confirm("确定"+text+"这个用户？",function(){
					var load = layer.load();
					$.post('/api/member/forbidUserById',{status:status,id:id},function(result){
						layer.close(load);
						if(result && result.status != 200){
							return layer.msg(result.message,so.default),!0;
						}else{
				...
				<form method="post" action="" id="formId" class="form-inline">
					<div clss="well">
						<div class="form-group">
							<input type="text" class="form-control" style="width: 300px;" value="${findContent?default('')}" name="findContent"
							 id="findContent" placeholder="输入昵称 / 帐号">
						</div>
						<span class="">
							<#--pull-right -->
