					...
					<div clss="well">
						<div class="form-group">
							<input type="text" class="form-control" style="width: 300px;" value="${findContent?default('')}" name="findContent"
							 id="findContent" placeholder="输入权限名称">
						</div>
						<span class="">

							<button type="submit" class="btn btn-primary">查询</button>
							<@shiro.hasPermission name="/api/permission/addPermission">
								<a class="btn btn-success" onclick="$('#addPermission').modal();">增加权限</a>
							</@shiro.hasPermission>
							<@shiro.hasPermission name="/api/permission/deletePermissionById">
	...
	<script>
		...
		function deleteById(ids) {
			var index = layer.confirm("确定这" + ids.length + "个权限？", function () {
				var load = layer.load();
				$.post('/api/permission/deletePermissionById', {
					ids: ids.join(',')
				}, function (result) {
					layer.close(load);
