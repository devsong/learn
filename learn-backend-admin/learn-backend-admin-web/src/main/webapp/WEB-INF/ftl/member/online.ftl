...
<html lang="zh-cn">
	<head>
		...
		<script >
			<@shiro.hasPermission name="/api/member/changeSessionStatus">
			$(function(){
			...
			function changeSessionStatus(sessionIds,status,self){
				...
				//loading
				var load = layer.load();
				$.post("${basePath}/api/member/changeSessionStatus",{status:status,sessionIds:sessionIds},function(result){
					layer.close(load);
