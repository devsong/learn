...
<html lang="zh-cn">
	<head>
		...
		so.init(function(){
				...
				<@shiro.hasPermission name="/api/role/clearRoleByUserIds">
				//全选
			...
			function deleteById(ids){
				var index = layer.confirm("确定清除这"+ ids.length +"个用户的角色？",function(){
					var load = layer.load();
					$.post('/api/role/clearRoleByUserIds',{userIds:ids.join(',')},function(result){
						layer.close(load);
			...
			<@shiro.hasPermission name="/api/role/addRole2User">
			<#--选择角色后保存-->
			function selectRole(){
				...
				var index = layer.confirm("确定操作？",function(){
					<#--loding-->
					var load = layer.load();
					$.post('/api/role/addRole2User',{ids:ids.join(','),userId:$('#selectUserId').val()},function(result){
						layer.close(load);
						if(result && result.status != 200){
			...
			*/
			function selectRoleById(id){
				var load = layer.load();
				$.post("/api/role/selectRoleByUserId",{id:id},function(result){
					layer.close(load);
